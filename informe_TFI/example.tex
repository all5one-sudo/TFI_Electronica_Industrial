...
\begin{figure}
	\centering
	\includegraphics[width=0.8\linewidth]{../schematic_mod}
...
\begin{enumerate}
	\item Se modifica el punto de conexión de la tensión de entrada, se utiliza la parte superior al capacitor $C_U$. 
	\item Se modifican los terminales de conexión del devanado primario del transformador.
	\item Se agrega un diodo volante a la inductancia $L_{in}$.
	\item Resistores en serie con los capacitores del sistema.
	\item Eliminación de las inductancias parásistas del transformador.
	\item Eliminación de los capacitores de de snubber.
\end{enumerate}

Los primeros 2 cambios van de la mano. Esa modificación a la topología se hizo para lograr el funcionamiento de la simulación de acuerdo con el diseño planteado a partir de la bibliografía. En este caso, cabe resaltar que se tiene la siguiente secuencia de funcionamiento en el devanado primario.

\begin{enumerate}
	\item Al cerrarse $S_U$, la corriente circula entrante por el terminal superior del devanado primario del transformador, y, al circular la corriente luego por $C_U$, éste se carga.
	\item Al conmutar, cerrándose $S_L$, el capacitor $C_L$ aporta la tensión a la que se cargó en el ciclo anterior, brindando así una corriente de entrada por el borne inferior del devanado primario del transformador.
\end{enumerate}

Se logra de esa forma la circulación de una tensión alternada, fundamental para el uso del transformador, que se encarga de transmitir tensiones alternadas entre devanados. En este caso se usa el efecto transformador para elevar la tensión y obtener una salida de mayor nivel de tensión.
...
Finalmente, por el hecho de la modificación de la red de alimentación no es necesaria la aplicación de capacitores de Snubber, ya que estos se encuentran directamente relacionados con el hecho de que se busca una tensión completamente positiva en el devanado primario. La topología adoptada permite excursionar de forma negativa y positiva en el transformador, lo que hace que el uso de los mismos no tiene sentido práctico ya.
